...
trait LoopsExp extends Loops with BaseExp with EffectExp {
  ...
  abstract class AbstractLoop[A] extends Def[A] {
    val size: Exp[scala.Int]
    val v: Sym[scala.Int]
    val body: Def[A]
  }
  
  case class SimpleLoop[A](val size: Exp[scala.Int], val v: Sym[scala.Int], val body: Def[A]) extends AbstractLoop[A]
  
  def simpleLoop[A:Manifest](size: Exp[scala.Int], v: Sym[scala.Int], body: Def[A])(implicit pos: SourceContext): Exp[A] = SimpleLoop(size, v, body)
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case SimpleLoop(s,v,body: Def[A]) => simpleLoop(f(s),f(v).asInstanceOf[Sym[scala.Int]],mirrorFatDef(body,f))
    case Reflect(SimpleLoop(s,v,body: Def[A]), u, es) if u == Control() => reflectMirrored(Reflect(SimpleLoop(f(s),f(v).asInstanceOf[Sym[scala.Int]],mirrorFatDef(body,f)), mapOver(f,u), f(es)))(mtype(manifest[A]), pos) 
...
trait LoopsFatExp extends LoopsExp with BaseFatExp {
  ...
  case class SimpleFatLoop(val size: Exp[scala.Int], val v: Sym[scala.Int], val body: List[Def[Any]]) extends AbstractFatLoop
